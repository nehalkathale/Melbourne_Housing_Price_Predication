...
import pandas as pd
data = main.loadData()
st.markdown("<h1 style='text-align: center; color: skyblue;'>Number of Houses by Types and Region</h1><br>", unsafe_allow_html=True)
...
with col1:
    ...
    color=alt.Color(field="Type", type="nominal",scale=alt.Scale(scheme='darkblue')),
...
with col2:
    with st.container():
        ...
        tooltip=['Number of Houses:Q'],
        color=alt.Color(field="Number of Houses",type="nominal",scale=alt.Scale(scheme='darkblue'),sort= "descending")
        ).properties(
            width=500,
            height=500,
...
st.markdown("<h1 style='text-align: center; color: skyblue;'>Mean Price by Month and Suburb</h1><br>", unsafe_allow_html=True)
...
with col3:
    with st.container():
        ...
        lineChartC = alt.Chart(average_house_value_year).mark_area(color="lightblue",line=True).encode(
            x='Year',
            y='Mean Price',
...
with col4:
    with st.container():
        ...
        top10Chart = alt.Chart(
            ...
            y=alt.Y('Price:Q'),
            color=alt.Color('Price:Q',scale=alt.Scale(scheme='darkblue'),sort= "descending"),
            tooltip=['Price:Q']
        ).transform_window(
            rank='rank(Suburb)',
...
with st.container():
    ...
    for y_encoding in ['Price:Q']:
        row = alt.hconcat()
        for x_encoding in ['Rooms:Q', 'Bedroom2:Q','Car:Q','Type:N']:
            row |= base.encode(x=x_encoding, y=y_encoding,tooltip=['Price:Q'],)
        chart &= row
    st.write(chart)